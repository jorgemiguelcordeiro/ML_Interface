...
def input_page():
    """
    This function includes all dictionaries, preprocessing functions, 
    grouping functions, and the final Streamlit form for user input. 
    It first extracts train_data.csv from train_data.zip, loads it, 
    prepares the data and the model, then displays a form for predictions.
    """

    # --------------------------------------------------------------------------------
    # 1) Define all mapping dictionaries (NAICS, nature_of_injury, cause_map, etc.)
    # --------------------------------------------------------------------------------
    ...
    nature_of_injury_mapping = {
        ...
        'Nature of Injury Cluster 16': [
            'MULTIPLE INJURIES INCLUDING BOTH PHYSICAL AND PSYCHOLOGICAL', 'MENTAL STRESS',
            'ALL OTHER SPECIFIC INJURIES, NOC', 'INFLAMMATION', 'ALL OTHER CUMULATIVE INJURY, NOC',
            'CONTUSION'
        ],
        'Nature of Injury Cluster 12': [
            'SPRAIN OR TEAR', 'CONCUSSION', 'MULTIPLE PHYSICAL INJURIES ONLY', 'STRAIN OR TEAR',
            'DUST DISEASE, NOC'
        ],
        'Nature of Injury Cluster 0': ['ASBESTOSIS'],
        'Nature of Injury Cluster 2': [
            'COVID-19', 'VISION LOSS', 'FREEZING', 'BURN', 'AIDS', 'ELECTRIC SHOCK', 'INFECTION', 'LACERATION',
        ...
        'Nature of Injury Cluster 5': [
            'ANGINA PECTORIS', 'POISONING - METAL', 'MYOCARDIAL INFARCTION',
    ...
    # & https://www.mwcia.org/Media/Default/PDF/NewsFeed/Circulars/21-1787.pdf
    WCIO_Cause_map = {
        'Cause of Injury Cluster 11': ['CRASH OF RAIL VEHICLE'],
        'Cause of Injury Cluster 15': [
            'FROM LADDER OR SCAFFOLDING', 'MOTOR VEHICLE', 'VEHICLE UPSET',
            'COLLISION OR SIDESWIPE WITH ANOTHER VEHICLE', 'MOTOR VEHICLE, NOC'
        ],
        'Cause of Injury Cluster 10': ['GUNSHOT'],
        'Cause of Injury Cluster 0': [
            'COLLAPSING MATERIALS (SLIDES OF EARTH)', 'MOVING PARTS OF MACHINE',
        ...
        'Cause of Injury Cluster 14': [
            'INTO OPENINGS', 'LIFTING', 'ON STAIRS', 'CRASH OF AIRPLANE', 'HOLDING OR CARRYING',
        ...
        'Cause of Injury Cluster 8': [
            'EXPLOSION OR FLARE BACK', 'CRASH OF WATER VEHICLE', 'FIRE OR FLAME',
        ...
        'Cause of Injury Cluster 13': [
            'PANDEMIC', 'OTHER - MISCELLANEOUS, NOC', 'OTHER THAN PHYSICAL CAUSE OF INJURY',
            'ABSORPTION, INGESTION OR INHALATION, NOC'
        ],
        'Cause of Injury Cluster 3': [
            'BROKEN GLASS', 'HOT OBJECTS OR SUBSTANCES', 'TEMPERATURE EXTREMES', 'STEPPING ON SHARP OBJECT',
    ...
    part_of_body_mapping = {
        ...
        'Part Of Body Cluster 4': [
            'CHEST', 'SKULL', 'BUTTOCKS', 'SOFT TISSUE', 'MULTIPLE HEAD INJURY',
            'ABDOMEN INCLUDING GROIN', 'LOWER LEG', 'LOWER ARM', 'HAND', 'FACIAL BONES',
            'FOOT', 'GREAT TOE', 'MULTIPLE LOWER EXTREMITIES', 'NOSE', 'SACRUM AND COCCYX',
            'TOES', 'UPPER LEG'
        ],
        'Part Of Body Cluster 2': [
            'MULTIPLE BODY PARTS (INCLUDING BODY', 'MULTIPLE NECK INJURY', 'LOWER BACK AREA',
    ...
    # Ordinal columns
    ordinal_columns = [
        "delay_days_category", "IME-4 Count Category", "Wage Category",
        "missing_info_category", "Number of Dependents"
    ]

    # One-hot columns
    one_hot_columns = [
        "Medical Fee Region", "Missing_Dates", "Mapped Industry Code Description",
        "Mapped WCIO Nature of Injury Description", "Mapped WCIO Cause of Injury Description",
        "Mapped WCIO Part Of Body Description", "Mapped Carrier Type"
    ]
    ...
    # --------------------------------------------------------------------------------
    # 2) Define the entire preprocessing_pipeline function
    #    (including everything from your snippet: helper functions, etc.)
    # --------------------------------------------------------------------------------
    def preprocessing_pipeline(train, val=None, test=None, outlier_treatment=True):
        """
        This function processes the training, validation, and/or test sets by:
         - Converting certain columns to numeric or binary
         - Replacing zeros
         - Handling missing data with KNN and mode-based imputers
         - Winsorizing outliers
         - Adding COVID-19 period flags
         - Mapping codes to custom categories (Industry Code, etc.)
        ...
        cols_to_impute = {
            'mode': [[
                'Gender', 'Alternative Dispute Resolution', 'Industry Code Description',
        ...
        columns_to_remove = [
            ...
            'WCIO Nature of Injury Description', 'WCIO Part Of Body Description', 'Carrier Type'
        ]
        ...
        def add_covid_flag(df, covid_start, covid_end):
            df['COVID Period'] = (
                (df['Accident Date'] >= covid_start) &
                (df['Accident Date'] <= covid_end)
            ).astype(int)
            return df
        ...
        def process_missing_values(df, is_train=False, cols_to_impute=None,
                                   imputers=None, scalers=None):
            if imputers is None:
                imputers = {'mode': {}, 'knn': {}}
            if scalers is None:
                scalers = {}
        ...
        def process_gender_and_alternative_dispute(df):
            ...
            if 'Alternative Dispute Resolution' in df.columns:
                df['Alternative Dispute Resolution'] = np.where(
                    df['Alternative Dispute Resolution'] == 'U',
                    np.nan,
                    df['Alternative Dispute Resolution']
                )
            return df

        # Helper for categorizing days
        ...
        # Helper for categorizing missing info
        def categorize_missing_info(df):
            ...
            df['missing_info_category'] = df.isnull().sum(axis=1).apply(categorize)
            debug_step("categorize_missing_info", df)
            return df

        # Helper for wage category
        ...
        # Helper to check missing date columns
        ...
        # Helper to validate date ordering
        def validate_dates(df):
            df["Assembly Date before Accident Date"] = df.apply(
                lambda row: (
                    1 if pd.notna(row["Accident Date"]) and
                         pd.notna(row["Assembly Date"]) and
                         row["Assembly Date"] < row["Accident Date"] else 0
                ),
                axis=1
            )
            df["C-2 or C-3 Date before Accident Date"] = df.apply(
                lambda row: (
            ...
            df["C-2 or C-3 Date after Assembly Date"] = df.apply(
                lambda row: (
        ...
        def process_dataset(df, is_train=False, mappings=None, bounds=None,
                            covid_start=None, covid_end=None, cols_to_impute=None,
                            imputers=None, scalers=None):
            ...
            if is_train:
                df, imputers, scalers = process_missing_values(
                    df, is_train=True, cols_to_impute=cols_to_impute, imputers=None, scalers=None
                )
                # Create mappings from code_col to desc_col
                map_store = {}
                for code_col, desc_col in description_columns:
                    map_store[desc_col] = df.groupby(desc_col)[code_col].apply(lambda x: x.value_counts().idxmax()).to_dict()
                    df[code_col] = df[desc_col].map(map_store[desc_col])

                if outlier_treatment:
                    df, bounds = winsorize_with_iqr(df, variables_to_treat_outliers, iqr_threshold)
                else:
                    bounds = {}
                return df, imputers, scalers, map_store, bounds
            else:
                if imputers is None or scalers is None:
                    raise ValueError("Imputers and scalers must be provided for validation/test.")
                df, _, _ = process_missing_values(df, is_train=False, cols_to_impute=cols_to_impute,
                                                  imputers=imputers, scalers=scalers)
                if mappings is None:
                    raise ValueError("Mappings must be provided for val/test.")
                for code_col, desc_col in description_columns:
                    df[code_col] = df[desc_col].map(mappings[desc_col])
                if outlier_treatment:
                    if bounds is None:
                        raise ValueError("Bounds must be provided for val/test.")
        ...
        train, imputers, scalers, code_mappings, bounds = process_dataset(
            train, is_train=True,
            covid_start=covid_start, covid_end=covid_end,
            cols_to_impute=cols_to_impute
        )

        # Process val & test if provided
        if val is not None:
            val = process_dataset(val, is_train=False, mappings=code_mappings,
                                  bounds=bounds, covid_start=covid_start,
                                  covid_end=covid_end, cols_to_impute=cols_to_impute,
                                  imputers=imputers, scalers=scalers)
        if test is not None:
            test = process_dataset(test, is_train=False, mappings=code_mappings,
                                   bounds=bounds, covid_start=covid_start,
                                   covid_end=covid_end, cols_to_impute=cols_to_impute,
                                   imputers=imputers, scalers=scalers)
        ...
        # Return the appropriate processed data
    ...
    # --------------------------------------------------------------------------------
    # 3) Define the entire "apply_groupings" function 
    #    (for region grouping, c2 grouping, etc.)
    # --------------------------------------------------------------------------------
    def map_region(row):
    ...
    def apply_groupings(df):
        """
        Main function to apply all groupings to the relevant columns.
        """
        ...
        # Combine two date checks into a single column
    ...
    # --------------------------------------------------------------------------------
    # 4) Define the "encoder_and_scaler" and "reduce_memory_usage" functions
    # --------------------------------------------------------------------------------
    def encoder_and_scaler(scaler, fit_data, df, ordinal_columns, one_hot_columns, numerical_columns):
        ...
        scaler_obj = scalers_dict[scaler]
        ...
        if numerical_columns:
            scaler_obj.fit(fit_data[numerical_columns])

        # Apply transformations
        ordinal_encoded = (
            ordinal_encoder.transform(df[ordinal_columns])
            if ordinal_columns else None
        )
        one_hot_encoded = (
            one_hot_encoder.transform(df[one_hot_columns])
            if one_hot_columns else None
        )
        scaled_numerical = (
            scaler_obj.transform(df[numerical_columns])
        ...
        one_hot_columns_names = (
            ...
            if one_hot_columns else []
        )

        transformed_dfs = []
        if ordinal_encoded is not None:
            transformed_dfs.append(
                pd.DataFrame(ordinal_encoded, columns=ordinal_columns, index=df.index)
            )
        if one_hot_encoded is not None:
            transformed_dfs.append(
                pd.DataFrame(one_hot_encoded, columns=one_hot_columns_names, index=df.index)
            )
        if scaled_numerical is not None:
            transformed_dfs.append(
                pd.DataFrame(scaled_numerical, columns=numerical_columns, index=df.index)
            )
    ...
    def reduce_memory_usage(df, continuous_columns):
        """
        Optimize the dataframe's memory usage.
        Continuous columns are treated as float32 or float64, 
        and other columns are treated as int32 or int64.
        """
        ...
        for col in df.columns:
            ...
            if col_type != object and col_type.name != 'category':
                if col in continuous_columns:
                    # Continuous columns => float
                    if c_min > np.finfo(np.float32).min and c_max < np.finfo(np.float32).max:
                        df[col] = df[col].astype(np.float32)
                    else:
                        df[col] = df[col].astype(np.float64)
                else:
                    # Discrete columns => int
    ...
    # --------------------------------------------------------------------------------
    # 5) Define the "extract_column_info" function
    # --------------------------------------------------------------------------------
    ...
    # --------------------------------------------------------------------------------
    # 6) Define the "preprocess_input" function
    # --------------------------------------------------------------------------------
    def preprocess_input(train, interface_data, features,
        ...
        """
        # 1) Use our pipeline to transform train & test
        train_processed, interface_data_processed = preprocessing_pipeline(
            train, test=interface_data, val=None, outlier_treatment=outlier_treatment
        )

        # 2) Apply grouping & transform
        train_to_scale = apply_groupings(train_processed)
        interface_data_to_scale = apply_groupings(interface_data_processed)
        ...
        train_encoded = encode_and_scale(train_to_scale, train_to_scale)
        interface_data_encoded = encode_and_scale(train_to_scale, interface_data_to_scale)

        train_encoded = reduce_memory_usage(train_encoded, numerical_columns)
        interface_data_encoded = reduce_memory_usage(interface_data_encoded, numerical_columns)

        # 3) Select features
        train_encoded = train_encoded[features]
        interface_data_encoded = interface_data_encoded[features]

        return interface_data_encoded

    # --------------------------------------------------------------------------------
    # 7) Extract train_data.csv from train_data.zip, then load, split, load model, etc.
    # --------------------------------------------------------------------------------
    ...
    try:
        ...
        st.warning(f"Could not display dataset info. Error: {e}")

    # Set index
    if 'Claim Identifier' not in train_data.columns:
        st.error("Expected 'Claim Identifier' column is missing from the dataset.")
        return
    train_data.set_index('Claim Identifier', inplace=True)

    # Identify columns that are fully empty for certain rows
    ...
    # Sometimes there's a known column to drop
    if 'OIICS Nature of Injury Description' in train_to_split.columns:
    ...
    for tgt in target_features:
        if tgt not in train_to_split.columns:
            st.error(f"Expected target column '{tgt}' not found in data. Cannot proceed.")
            return
    ...
    # Load your trained model (Replace with the actual file name if different)
    try:
        model = joblib.load('logistic_model.joblib')
        st.success("Trained model loaded successfully.")
    except FileNotFoundError:
        st.error("trained_model.pkl not found. Please place your model file in the same directory.")
        return
    except Exception as e:
    ...
    # --------------------------------------------------------------------------------
    # 8) Build the user input form in Streamlit
    # --------------------------------------------------------------------------------
    st.header("Enter Claim Details")

    # We gather column_info for dynamic form building
    ...
    for col, info in column_info.items():
        if isinstance(info, list):
            # Categorical => show a selectbox
            input_data[col] = st.selectbox(f"{col} (Optional)", [""] + info)
        elif isinstance(info, tuple):
            # Numeric => min/max
            min_val, max_val = info
            input_data[col] = st.number_input(
                f"{col} (Optional, Range: {min_val} - {max_val})",
                value=0.0, step=1.0
            )
        elif info == "Date format (YYYY-MM-DD)":
            # Date => text input
            input_data[col] = st.text_input(
                f"{col} (Enter as YYYY-MM-DD or leave blank)",
                value=""
            )
        else:
            # Fallback => text
            input_data[col] = st.text_input(f"{col} (Optional)", value="")
